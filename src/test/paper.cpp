...
void calPrecision(int N, Bipartite* bigraph, RandomWalk* walker, set<string>& matches, double eta)
{
...
void outputFinal(int N, Bipartite* bigraph, RandomWalk* walker, set<string>& matches, double eta)
{
	int Num=1000;
	double max_weight=-1;
	...
	for(int i=Num-1;i>=0;i--){
		for(size_t j=0;j < buckets[i].size();j++){
			...
			stringstream ss;
			ss<<id1<<"_"<<id2;
			cout<<id1<<"\t"<<id2<<"\t"<<walker->p_conf[id1*N+id2]<<"\t"<<bigraph->p_score[id1*N+id2]<<"\t"<<score_threshold<<"\t";
			if(walker->p_conf[id1*N+id2]>=eta && bigraph->p_score[id1*N+id2]>= score_threshold){
				cout<<"\t11111\t";
			}
			if(matches.find(ss.str()) != matches.end()){
...
int main(int argc, char** argv)
{
	...
	for(int iter=0;;iter++){
		...
		walker = new RandomWalk(bigraph->p_score, N, alpha, 60, S);
		for(int i=0;i < N;i++){
			for(int j=i+1;j < N;j++){
