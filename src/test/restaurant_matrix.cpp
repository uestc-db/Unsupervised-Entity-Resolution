...
void outputFinal(int N1, int N2, int N, Bipartite* bigraph, CliqueRankMatrix* walker, set<string>& matches, double eta)
{
	int Num=1000;
	double max_weight=-1;
	...
	for(int i=Num-1;i>=0;i--){
		for(size_t j=0;j < buckets[i].size();j++){
			...
			stringstream ss;
			ss<<id1<<"_"<<(id2-N1);
			cout<<id1<<"\t"<<id2<<"("<<id2-N1<<")\t"<<walker->p_conf(id1,id2)<<"\t"<<bigraph->p_score[id1*N+id2]<<"\t"<<score_threshold<<"\t";
...
void calPrecision(int N1, int N2, int N, Bipartite* bigraph, CliqueRankMatrix* walker, set<string>& matches, double eta)
{
...
int main(int argc, char** argv)
{
	...
	//cout<<"edge num: "<<bigraph->activePairNum()<<endl;
        ...
        // CliqueRank: Edit by guolong
	set<string> matches = FileIO::readMatch("../../data/"+dataset+"/match.txt");
	CliqueRankMatrix* cwalker;
	for(int iter = 0;;iter++){
		cout<<"Iteration "<<iter<<endl;
		cwalker = new CliqueRankMatrix(bigraph->p_score, N1+N2, alpha, S, 0.1);
		for(int i=0;i < N1;i++){
			int id1=i;
			for(int j=0;j < N2;j++){
				int id2=j+N1;
		...
		calPrecision(N1, N2, N, bigraph, cwalker, matches, eta);
		...
		bigraph->updatePScore(cwalker->p_conf, N1+N2);
	}
	//outputFinal(N1, N2, N, bigraph, cwalker, matches, eta);
